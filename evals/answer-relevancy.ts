...
export const checkAnswerRelevancy = async (opts: {
  ...
  const { object: statementsResult } = await generateObject({
    ...
    schema: z.object({
      statements: z.array(z.string()).describe("Array of meaningful statements from the output"),
    }),
  });

  // Handle edge case of empty statements
  const statements = statementsResult.statements.length === 0 ? [""] : statementsResult.statements;
  ...
  const averageScore = statementScores.length > 0 
    ? statementScores.reduce((sum, score) => sum + score, 0) / statementScores.length
    : 0;
...
>({
  ...
  scorer: async ({ input, output }) => {
    // Extract the question text from the UIMessage format
    const firstPart = input[0]?.parts?.[0];
    const question = (firstPart?.type === "text" && "text" in firstPart) 
      ? firstPart.text 
      : "Unknown question";
